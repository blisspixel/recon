"""CAL3 / CAL4: calibrate a posterior against a public oracle (the record
...
7489). The DMARC record is its own ground truth, so it is an *oracle*
the posterior can be calibrated against, not merely a self-consistency
check.

What this measures, and what it does not. The oracle label here comes
from an authoritative external definition (the published DMARC policy
level), not from recon's own posterior, so this is stronger than the
...
evidence includes the DMARC signal, so the posterior and the oracle
overlap on input. What it does test honestly: whether recon's
*combined* multi-signal posterior (DMARC plus strict SPF plus MTA-STS,
under the node's definition) is *calibrated* against the authoritative
DMARC-only definition. Where recon over-weights SPF or MTA-STS and
reports a high posterior on a domain whose DMARC is ``p=none``, the
oracle catches it.

Why calibration and not interval coverage. The oracle label is binary
...
this harness supplies the real-oracle calibration that the synthetic one
cannot.
...
(tests/test_oracle_calibration.py); the orchestration is the
maintainer-run part.

Run (maintainer-local, network):

    python -m validation.oracle_calibration domains.txt
    python -m validation.oracle_calibration domains.txt --bins 10 --concurrency 5
"""
...
def oracle_label_email_policy(dmarc_policy: str | None) -> int | None:
    ...
    record was observed, in which case the domain carries no oracle truth
    and is excluded from the calibration rather than guessed.
    """
...
class OracleRecord:
    """One domain's posterior and its oracle label for a node.
...
def calibration_summary(records: list[OracleRecord], bins: int = 10) -> dict[str, object]:
    """Aggregate calibration of the posteriors against the oracle labels.
...
async def _collect_one(domain: str, *, timeout: float, skip_ct: bool, sem: asyncio.Semaphore) -> OracleRecord | None:
    """Resolve one domain, infer, and pair the policy posterior with the
    oracle label. Returns None when the domain has no DMARC oracle truth.
    ...
    async with sem:
        try:
            ...
            return None
    label = oracle_label_email_policy(getattr(info, "dmarc_policy", None))
    if label is None:
        return None
    ...
    if node is None:
        return None
    return OracleRecord(posterior=float(node.posterior), label=label)


async def collect(domains: list[str], *, timeout: float, skip_ct: bool, concurrency: int) -> list[OracleRecord]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Calibrate the email-policy posterior against the DMARC oracle.")
    ...
    print(f"Resolving {len(domains)} domains for the DMARC oracle (aggregates only, no apex printed)...")
    ...
    if summary["n"] == 0:
        print("No domains carried a DMARC oracle label; nothing to calibrate.")
        return 0

    print(f"\nEmail-policy node calibrated against the DMARC oracle (n={summary['n']} with a published policy)")
    ...
    print("\nThis is oracle-anchored calibration (the label is the authoritative DMARC")
    print("policy, an external definition), stronger than tier-2 consistency but not the")
