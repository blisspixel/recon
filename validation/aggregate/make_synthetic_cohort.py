...
_BRANDS = [
    "contoso", "northwind", "fabrikam", "adventureworks", "wingtiptoys",
    "tailspintoys", "proseware", "fourthcoffee", "litware", "alpineskihouse",
    "wideworldimporters", "blueyonder", "graphicdesigninstitute", "lucernepublishing",
    "margiestravel", "trey-research", "vanarsdel", "woodgrovebank", "consolidated-messenger",
    "famfields", "relecloud", "southridgevideo", "tailwindtraders", "coho-vineyard",
]
...
_PROFILES: dict[str, dict[str, Any]] = {
    "fintech": {
        "m365": 0.85, "dmarc_reject": 0.7, "dmarc_quarantine": 0.15, "gateway": 0.6,
        "mta_sts": 0.45, "cloud": {"Azure": 0.5, "AWS": 0.4, None: 0.1},
        "slugs": ["microsoft365", "entra-id", "exchange-online", "okta", "proofpoint",
                  "stripe", "docusign", "splunk"],
        "thin_rate": 0.15,
    },
    "healthcare": {
        "m365": 0.6, "dmarc_reject": 0.3, "dmarc_quarantine": 0.3, "gateway": 0.4,
        "mta_sts": 0.2, "cloud": {"Azure": 0.45, "AWS": 0.25, None: 0.3},
        "slugs": ["microsoft365", "exchange-online", "mimecast", "epic-mychart",
                  "servicenow", "zoom"],
        "thin_rate": 0.3,
    },
    "saas": {
        "m365": 0.3, "dmarc_reject": 0.65, "dmarc_quarantine": 0.2, "gateway": 0.2,
        "mta_sts": 0.5, "cloud": {"AWS": 0.6, "GCP": 0.3, None: 0.1},
        "slugs": ["google-workspace", "cloudflare", "okta", "fastly", "github",
                  "segment", "datadog", "hubspot"],
        "thin_rate": 0.2,
    },
}
...
def _node(rng: random.Random, name: str, present: bool, *, declarative: bool = False,
          thin: bool = False) -> dict[str, object]:
    ...
    if present and thin:
        ...
        return {"name": name, "description": "", "posterior": post, "interval_low": max(0.0, low),
                "interval_high": min(1.0, high), "evidence_used": [f"slug:{name}"],
                "n_eff": round(rng.uniform(1.5, 3.0), 2), "sparse": False}
    if present:
        post = round(rng.uniform(0.86, 0.97), 3)
        return {"name": name, "description": "", "posterior": post,
    ...
    return {"name": name, "description": "", "posterior": post,
            ...
            "evidence_used": [], "n_eff": round(rng.uniform(0.5, 1.2), 2), "sparse": True}
...
def _record(rng: random.Random, brand: str, group: str) -> dict[str, object]:
    ...
    dmarc = "reject" if rng.random() < prof["dmarc_reject"] else (
        "quarantine" if rng.random() < prof["dmarc_quarantine"] else
        ("none" if rng.random() < 0.5 else None))
