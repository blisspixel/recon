# Oracle calibration against the DMARC record (CAL3 / CAL4)

The roadmap's highest-value open assurance item: calibrate a recon
posterior against an external public oracle, the one place the passive
setting allows it. Harness: `validation/oracle_calibration.py`. Unit
tests for the pure logic: `tests/test_oracle_calibration.py`. The
...
ground-truth oracle, and the harness pairs, per domain, recon's policy
posterior with the binary oracle label, then reports aggregate
...
The oracle label is binary, and a credible interval is for the
...
The oracle label comes from an authoritative external definition (the
...
DMARC signal, so the posterior and the oracle overlap on input. What it
tests honestly: whether recon's combined multi-signal posterior (DMARC
...
recon's posterior high; the oracle marks that a miss.
...
```
python -m validation.oracle_calibration validation/corpus-private/consolidated.txt
```
...
numbers are invented for the format only; the real run replaces them.

```
Email-policy node calibrated against the DMARC oracle (n=420 with a published policy)
...
The harness and its unit tests ship now. The calibration *run* against
the corpus is maintainer-local and produces an aggregate-only follow-up
memo; until then the dossier reports tier 4 as the open frontier for
this node.
