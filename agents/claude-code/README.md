...
- **The skill** teaches Claude when to reach for recon and how to read its hedged output. Driving the CLI, it covers the one-shot analyses: the lookup, the model-bound public-evidence index (`recon <domain> --exposure --json`), the hardening observations (`recon <domain> --gaps --json`), and the Bayesian posteriors (`--fusion`, `--explain-dag`). For most presales and vendor-diligence work, that is the whole job, with zero setup.
