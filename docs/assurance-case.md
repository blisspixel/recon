...
| Suppression guarantee (the one formal theorem): an operator who hides indicators can only move a claim toward "we cannot tell," never to a confident false positive; proved in correlation.md 4.3 and machine-checked exhaustively over every per-node binding subset | `validation/adversarial_properties.py`; gated by `tests/test_adversarial_properties.py` (and the positive-indicator hypothesis it rests on) | **Provably robust to hiding, explicitly not robust to lying:** the bound covers the *removal* of evidence, not the *fabrication* of it. A published fake indicator raises the posterior on a false premise, and catching it needs active verification the passive invariant forbids (deception postures, correlation.md 4.11) |
| The email-policy posterior is calibrated against the DMARC record on real domains (tier 4): two independent corpus samples agree at ECE about 0.077, the miss conservative (under-confident) | `validation/reference_calibration.py`; memo `validation/reference-calibration.md`; dossier `statistical-assurance.md` | Only this one node has an external reference; the agreement is partly definitional (DMARC is also an input), so the reliability table and ECE are the load-bearing figures |
