...
| Deeply-nested or excessive-numeric JSON in a poisoned cache, delta snapshot, client config, or CT payload | readers catch parser recursion and numeric-limit failures, apply bounded pre-reads, and validate consumed field shapes | `test_resilience_hardening::TestPoisonedCacheDegrades`, `::test_rate_limit_load_persisted_degrades_on_poison`, `::TestCtProviderRecursionError`; `test_delta::TestLoadPrevious`; `test_doctor_client` | Local files can still change between open and read, but each reader consumes only its bounded handle |
...
| Suppression guarantee (the one formal proposition): an operator who hides indicators can only move a claim toward "we cannot tell," never to a confident false positive; proved in correlation.md 4.3 and machine-checked over every per-node binding subset under a sweep of external-evidence contexts | `validation/adversarial_properties.py`; gated by `tests/test_adversarial_properties.py` (and the positive-indicator hypothesis it rests on) | **Provably robust to hiding, explicitly not robust to lying:** the bound covers the *removal* of evidence, not the *fabrication* of it. A published fake indicator raises the posterior on a false premise, and catching it needs active verification the passive invariant forbids (deception postures, correlation.md 4.11) |
| The email-policy posterior is checked against the DMARC record on real domains (full-posterior calibration strong but DMARC-anchored; the clean residual disconfirmed): the full posterior agrees at fixed-bin ECE 0.0761 and equal-mass ECE 0.0651 over n=2,906 across 22 verticals, the miss conservative (under-confident) | `validation/reference_calibration.py`; memo `validation/reference-calibration.md` and `validation/2026-06-28-full-corpus-calibration-refresh.md`; dossier `statistical-assurance.md` | DMARC is also the node's dominant input, so the populated bins are a definitional agreement check; the full-corpus held-out run (the `dmarc_policy` unit masked so predictor and label are disjoint) shows the SPF/MTA-STS residual is weak and poorly calibrated (fixed-bin ECE 0.3747, equal-mass ECE 0.3263), so it is not a clean tier-4 result. The result is for a *declared* policy, not an enforced one, and is forgeable (Pattern I) |
| The 2.2 evidence-semantics diagnostics report exactly what the engine computes: every `unit_counterfactuals` entry equals the posterior of an actual leave-one-unit-out (`masked_units`) re-inference, per-node `entropy_reduction_nats` sums to the result-level total, and `partition_stability` is the seed-sweep ARI consensus | `tests/test_evidence_semantics_diagnostics.py` (the masked-run cross-check is the load-bearing pin); `tests/test_bayesian_masked_units.py` (mask semantics, incl. the network-deletion equivalence) | The counterfactuals are evidence counterfactuals over the model, never causal claims about the world; deltas are individually exact but not additive (units interact through the DAG), and the docstrings and schema say so |
