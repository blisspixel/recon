...
| Deeply-nested or excessive-numeric JSON in a poisoned cache, delta snapshot, client config, or CT payload | readers catch parser recursion and numeric-limit failures, apply bounded pre-reads, and validate consumed field shapes; TenantInfo and CT cache round trips retain current certificate and infrastructure extensions | `test_resilience_hardening::TestPoisonedCacheDegrades`, `::test_rate_limit_load_persisted_degrades_on_poison`, `::TestCtProviderRecursionError`; `test_delta::TestLoadPrevious`; `test_doctor_client`; `test_cache_roundtrip::TestCacheDiskOperations`; `test_ct_cache::TestCTCachePutGet` | Local files can still change between open and read, but each reader consumes only its bounded handle |
