...
| Malformed, oversized, swapped, symlinked, or modified observation capsule | 10 MiB stable regular-file read, structural validation, normalized-snapshot decode, per-fact digest, outer canonical digest, exclusive no-clobber reservation, and atomic write | `test_capsule`; `test_capsule_cli`; `test_capsule_schema` | SHA-256 detects unkeyed modification but does not authenticate the collector or signer; the caller controls the output directory |
...
| Observation capsules preserve caller-owned collection context and classify observation, collection-regime, time-evaluation, and interpretation changes without altering legacy delta or lookup JSON | `recon_tool.capsule`; `tests/test_capsule.py`; `tests/test_capsule_cli.py`; [observation-capsules.md](observation-capsules.md) | Source opportunity is currently role-level and may conservatively withhold a real channel change. Replay uses retained normalized state rather than complete raw HTTP bodies. No universal freshness predicate, authenticity guarantee, monitoring, or retention service is claimed |
...
| Mechanism | Proven by |
|---|---|
| Exact uv and a hash-locked Hatchling dependency graph constrain every release-shaped build; the committed constraints are a frozen PEP 735 build-group export and ship in the sdist | `test_build_constraints::test_build_root_and_uv_are_exactly_selected`; `test_build_constraints::test_build_constraints_are_exact_complete_and_hashed`; `test_build_constraints::test_build_constraints_match_frozen_build_group`; `test_build_constraints::test_artifact_workflows_select_reproducible_uv_version`; `test_package_invariants::test_sdist_retains_public_sources_and_excludes_private_artifacts` |
| Same-job deterministic-build check (`SOURCE_DATE_EPOCH`) creates the sdist, reconstructs its wheel, repeats the sequence, and matches both hashes; the built wheel then enters blocking cross-platform smoke | `ci.yml` `reproducible-build` and `installed-wheel-smoke` jobs; `test_check_script::test_reproducible_build_feeds_blocking_cross_platform_artifact_smoke` |
