...
| The email-policy posterior is checked against the DMARC record on real domains (tier 4 for the strict-SPF + MTA-STS residual only): two independent corpus samples agree at ECE about 0.077, the miss conservative (under-confident) | `validation/reference_calibration.py`; memo `validation/reference-calibration.md`; dossier `statistical-assurance.md` | DMARC is also the node's dominant input, so the populated bins are a definitional agreement check and only the SPF/MTA-STS residual is independently tested; the held-out residual mode now ships in the harness (the `dmarc_policy` unit masked so predictor and label are disjoint) and its maintainer-local run is the open item. The result is for a *declared* policy, not an enforced one, and is forgeable (Pattern I) |
