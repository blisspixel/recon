...
| Entropy reduction and posture stratification summarize what the public channel still leaks | Aggregate-only private memo, disclosure review still required | `validation/posture_distributions.py`, `tests/test_posture_distributions.py`, [paper-draft.md](paper-draft.md) section 6 | Keep as draft-local until the aggregate memo is disclosure-reviewed and linked. |
| Per-vertical residual failure generalizes across the curated cohort | Aggregate-only private memo, disclosure review still required | [paper-draft.md](paper-draft.md) section 6, private run aggregate | Keep caveated to the curated cohort. Do not generalize beyond that population. |
