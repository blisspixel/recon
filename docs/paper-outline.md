...
|---|---|---|
| Reference calibration (DMARC) | the email-policy full posterior agrees with the DMARC record (ECE 0.076, n=2,905 across 22 verticals, miss conservative); DMARC-anchored, since DMARC is also the dominant input | `validation/reference_calibration.py`; run 2026-06 |
| Held-out residual calibration | recompute the policy posterior with the DMARC unit masked as structurally unobserved (`masked_units`, not "absent", since the declarative node would read deletion as disconfirmation) and calibrate the residual against the DMARC label, predictor and label disjoint; the 2026-06 run shows the residual weak and poorly calibrated (ECE 0.373), so it disconfirms the clean tier-4 claim rather than establishing it | `validation/reference_calibration.py` (both modes print full + held-out blocks); run 2026-06 |
| Held-out residual diagnosis | after DMARC is masked, the remaining signal path is weak by construction: MTA-STS is rare even among enforcing domains, and strict SPF is only a supporting signal | `src/recon_tool/data/bayesian_network.yaml`; `validation/2026-06-23-full-corpus-calibration.md` |
| Tenancy corroboration (provider endpoints) | the M365 tenancy posterior computed from the DNS channel alone, calibrated against Microsoft's own endpoint attestation (two-class label: tenant ID / namespace positive, documented not-found negative), predictor and label disjoint by observation channel; GWS reported one-sided (recall on attested-federated) because the Google channel has no authoritative negative | `validation/tenancy_reference_calibration.py`; run 2026-06: ECE 0.048, agreement 0.889, n=3,309; GWS recall 0.58 (n=12) |
