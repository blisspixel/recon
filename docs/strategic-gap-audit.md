...
| Catalog quality and freshness | A large catalog can grow coverage and false positives at the same time. | The catalog has 850 entries and optional `verified` metadata, but no active classified-surface or stale-rule regression budget. | Baseline by record type and stratum, then ratchet references, dates, and negative fixtures. | No new undated, unreferenced, or untested rule. |
