...
| Catalog quality and freshness | A large catalog can grow coverage and false positives at the same time. | Shipped in v2.14. The catalog has 867 entries and 1,110 detections. The convenience, unseen-vertical, rank, regional, vendor-seed, and prior-sample drift rounds are closed with aggregate results and explicit dispositions. Drift measured all 5,199 rows, found no decline beyond the frozen threshold, disclosed one catalog-driven measurement-surface change, withheld classification comparison, and promoted no rule. The legacy date backlog remains open: 160 of 1,110 detections are dated (14.4 percent). | Keep frozen contracts reproducible and backfill dates only after independent review. The September claim review narrows Vercel routes, corrects Slack and Glitch semantics, and retires unsupported AWS, Okta, and GitHub product attributions. Earlier dated family reviews remain historical evidence, not grounds to preserve an incorrect rule. Proofpoint and unsupported variants stay undated until exact current first-party evidence exists. | No new undated or untested rule. No population claim from the convenience sample and no broad coverage claim while a bounded path or named stratum is unmeasured. |
