...
| Catalog quality and freshness | A large catalog can grow coverage and false positives at the same time. | The catalog has 855 entries and 1,062 detections. One frozen convenience-sample baseline covers every bounded path, and the first provider-supported promotion pass includes positive and deceptive-lookalike fixtures. The legacy date backlog and independent strata remain open. | Run the predeclared unseen vertical holdout, then add rank and regional rounds, backfill dates in reviewed families, and ratchet stale dates. | No new undated or untested rule. No population claim from the convenience sample and no broad coverage claim while a bounded path or named stratum is unmeasured. |
