...
context treatment, and packaging portability remains a real remaining gap. Its
preflight now passes, but paired collection is unmeasured and does not queue
product work. Agent Plugins v1.0.0 and Open Knowledge Format v0.2 are explicit interoperability inputs, but
...
The 5,199-row frozen-sample drift contract and observation-only comparison are
...
rules. A second network-free preflight on 2026-08-20 passed with recon 2.17.4
and all three frozen clients available; it started zero sessions and made zero
network requests. Paired collection still requires the frozen frame and a
declared cost envelope; it does not queue product work. The standing product
work is the fingerprint-freshness loop. No
...
- Agent Plugins v1.0.0 Published specification and compatible-client list:
...
| Catalog quality and freshness | A large catalog can grow coverage and false positives at the same time. | Shipped in v2.14. The catalog has 869 entries and 1,108 detections. The convenience, unseen-vertical, rank, regional, vendor-seed, and prior-sample drift rounds are closed with aggregate results and explicit dispositions. Drift measured all 5,199 rows, found no decline beyond the frozen threshold, disclosed one catalog-driven measurement-surface change, withheld classification comparison, and promoted no rule. The legacy date backlog remains open: 134 of 1,108 detections are dated (12.1 percent). | Keep the frozen contracts reproducible and backfill dates only in independently reviewed families after a current vendor-page confirmation. The 2026-08-19 passes dated Google Workspace and Microsoft 365 MX families, then the Okta custom-domain family. The 2026-08-20 passes dated Mimecast; current TrendAI regional MX and SPF values; Barracuda inbound MX; Cisco Cloud Gateway `iphmx.com`; all 22 current AWS SES receiving regions; current AWS SES, Google Workspace, and Microsoft 365 SPF values; four current Akamai edge CNAME suffixes; the exact current HubSpot SPF and `hubspot.net` CNAME forms; four exact current Marketo SPF, CNAME, and tracking-link forms; eight exact Salesforce Marketing Cloud CNAME or discovered-target roles; consolidated partition-aware AWS ELBv2 and API Gateway roles; and corrected Microsoft 365, Azure Communication Services Email, GCC High, Autodiscover, and SharePoint boundaries. Publish this batch through protected main next, then continue unsupported Microsoft and Proofpoint residuals only with exact current first-party evidence. | No new undated or untested rule. No population claim from the convenience sample and no broad coverage claim while a bounded path or named stratum is unmeasured. |
...
are complete. The first local preflight remains historical stop evidence. A
second preflight on 2026-08-20 passed with recon 2.17.4 and all three clients,
while starting zero sessions and making zero network requests. The paired frame
remains unmeasured and runs only under a declared cost envelope; it does not
