...
and a fixed-observation zero-regression decision. The regional source mapping
and 1,000-row frame are frozen before target contact; execute and disposition
that round next, then freeze and run vendor-seed and drift rounds with
...
6. Keep the shipped v2.13 caller-held observation capsule, four-way comparison,
   ...
   versioned JSON.
7. Preserve the closed v2.14 rank-round decision and frozen regional source and
   frame commitments, then execute and disposition regional before freezing
   and executing vendor-seed and drift with independent strata or the
...
explicit candidate dispositions. The regional contract is frozen and its
collection and disposition are next; each later vendor-seed and drift contract
must be fixed before collection and must end in aggregate evidence plus explicit
candidate dispositions. v2.12 has
