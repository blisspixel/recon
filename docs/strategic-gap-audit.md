...
| Catalog quality and freshness | A large catalog can grow coverage and false positives at the same time. | The catalog has 850 entries, 1,051 detections, a diff-aware date gate for new rules, and a large legacy date backlog. Only CNAME-chain gaps are ranked corpus-wide. | Baseline every bounded record path by type and stratum, backfill dates in reviewed families, then ratchet stale dates and negative fixtures. | No new undated or untested rule. No broad coverage claim while a bounded record path is unmeasured. |
