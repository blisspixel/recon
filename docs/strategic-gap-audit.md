...
Current catalog counts updated 2026-09-05 from the generated artifact; this
does not redate the external-source audit below. The subsequent
[claim review](../validation/2026-09-05-catalog-claim-review.md) records the
bounded corrections and retirements.
...
| Catalog quality and freshness | A large catalog can grow coverage and false positives at the same time. | Shipped in v2.14. The catalog has 870 entries and 1,114 detections. The convenience, unseen-vertical, rank, regional, vendor-seed, and prior-sample drift rounds are closed with aggregate results and explicit dispositions. Drift measured all 5,199 rows, found no decline beyond the frozen threshold, disclosed one catalog-driven measurement-surface change, withheld classification comparison, and promoted no rule. The legacy date backlog remains open: 167 of 1,114 detections are dated (15.0 percent). | Keep frozen contracts reproducible and backfill dates only after independent review. The September claim review narrows Vercel routes, corrects Slack and Glitch semantics, and retires unsupported AWS, Okta, and GitHub product attributions. A documented-target review adds exact Retool and Postmark indicators, then separately retires three unsupported legacy Postmark rules. Documented Tailscale, PostHog, and Resend rules follow the same owner-or-exact-target bar. Synthetic claim-boundary tests are not measured accuracy gains. Earlier dated reviews do not justify preserving an incorrect rule. Proofpoint and unsupported variants stay undated until exact current first-party evidence exists. | No new undated or untested rule. No population claim from the convenience sample and no broad coverage claim while a bounded path or named stratum is unmeasured. |
