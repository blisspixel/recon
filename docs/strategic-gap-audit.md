...
accepted. Freeze and run vendor-seed and drift rounds next with aggregate-only
evidence.
...
6. Keep the shipped v2.13 caller-held observation capsule, four-way comparison,
   ...
   versioned JSON.
7. Preserve the closed v2.14 rank and regional decisions, then freeze and
   execute vendor-seed and drift with independent strata or the frozen prior
   sample for drift, aggregate-only
...
is next; it and the later drift contract must be fixed before collection and
end in aggregate evidence plus explicit candidate dispositions. v2.12 has
