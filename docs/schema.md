...
| Field | Type | Nullable | Values | Stability | Description |
|---|---|---|---|---|---|
| `email_security_score` | int | no | `0-5` | stable | Compatibility count of five observed-present public controls: effectively enforcing DMARC, DKIM selectors, SPF strict (`-all`), MTA-STS in effect, and BIMI. Since v2.7.1 an MTA-STS policy fetched with RFC 8461 `mode: none` does not count, because that mode means the policy is not in effect; a domain that published and then disabled MTA-STS previously scored the same as one enforcing it. The `mta_sts_mode` field still reports `none`, so the observation stays visible. If a constituent channel is unavailable, the count is incomplete; consult `degraded_sources` rather than treating uncounted controls as completed negatives. |
...
| `fusion_enabled` | bool | no | n/a | stable (v2.0+) | True when the evidence-strength and Bayesian-network post-processing ran (SH6). v2 preserves the historical implicit enabled CLI default during the ADR-0013 transition; explicit advanced MCP tools are unchanged. When false, `slug_confidences` is an empty object and `posterior_observations` is an empty array. When true, `slug_confidences` can still be empty if no retained evidence carries a slug, while `posterior_observations` carries the network's fixed nodes. |
