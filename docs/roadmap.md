...
Current release: **v1.9.15** (close a second instance of the
"query and leak internal DNS names" finding: the SPF `redirect=`
chaser now validates each target against `_is_public_dns_name`
before resolving it, the same suffix denylist the CNAME walker
uses; the dormant A/AAAA helper removed in this release was the
last of the reverted v1.9.13 path). Fifth bridge release after
