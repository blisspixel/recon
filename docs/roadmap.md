...
| v0.4.0 | Hedged inference language, removed sys.argv hack, `recon mcp` subcommand, disk cache (`--no-cache`, `--cache-ttl`), `--csv` batch output, `doctor --fix` scaffolding. 597 tests |
...
- **Web dashboard / `recon serve`** — this is a CLI tool and MCP server. Markdown (`--md`) is the shareable report format.
- **HTML output** — markdown renders everywhere that matters (GitHub, VS Code, Obsidian, AI agents). HTML is bloat for a focused CLI tool.
