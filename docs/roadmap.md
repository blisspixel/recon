...
| UX flow | Root help, no-argument onboarding, malformed-input recovery, all-source failure recovery, low-confidence next steps, batch outcome guidance, cross-platform release verification, and target-free list-search-show catalog discovery are implemented (Unreleased) | Specify batch all-error exit semantics before considering an opt-in strict mode |
| Visual polish | Lookup and batch help use task panels; below 70 columns, linear help keeps long options and complete command summaries visible; narrow welcome descriptions remain attached to their commands; fingerprint search is a bounded unique-slug preview; fingerprint and signal rows keep labels associated when wrapped (Unreleased) | Preserve complete option visibility and field association before changing presentation metadata |
| Observability | MCP validation rejection logs are target-free; unexpected batch details are debug-only; doctor retains independent rows across ordinary HTTP request failures; `cache show` exposes payload-free metadata and reuse status for both disk layers; captured canonical-gate wrapper output is plain text; remote readiness names exact SBOM, provenance, signer, tag, commit-digest, metadata, and channel-parity outcomes; the partial-release runbook distinguishes transient reruns from permanent evidence failures (Unreleased) | Define a versioned doctor record before adding a machine-facing operator contract |
| Reliability | Typed batch errors, bounded workers, truthful cache inspection and clearing, closed-pipe handling, partial results, exact exit-code semantics, import-safe runtime-scoped MCP logging, shared CLI and MCP category matching, complete same-slug catalog inspection, explicit sdist-to-wheel reconstruction, an exact sealed artifact pair, PyPI-to-GitHub byte parity, and fail-closed existing-release recovery with strict operator preconditions are implemented and listed in Unreleased | Do not change the current batch exit contract without a compatibility decision covering mixed and all-error streams |
| Security | Unresolved HTTP destinations fail closed; rejected MCP arguments and unexpected batch details stay out of default output; fatal MCP, live MCP, installer, doctor, persisted-cache, and locally extended catalog detail text cross bounded terminal boundaries; artifact builds use exact uv and a hash-locked backend graph; tag helpers preserve one owner and exact version; release verification reuses bounded exact PyPI metadata validation and binds the current workflow's wheel, sdist, and completed SBOM to the exact workflow, tag, and commit digest, with one digest-bound v2.6.3 historical exception (Unreleased) | Any exact configured-launcher doctor must require a safe explicit execution boundary |
| Accessibility | `--plain` is shipped; complete narrow help uses the actual terminal width without literal source markup or truncated root or nested summaries; the no-argument path and catalog rows preserve field association when wrapped (Unreleased) | Keep both paths complete; do not replace the parser or hide specialist controls |
| Documentation accuracy | README helper discovery follows exact release-tag review; target-free catalog discovery, generated CLI reference, operational contract, automation examples, security model, constrained rebuild recipe, fail-closed POSIX and PowerShell consumer verification, exact-tag acquisition, and strict partial-publication recovery instructions track current behavior (Unreleased) | Keep the eventual versioned doctor record separate from human diagnostic output |
