...
| Documentation accuracy | README helper discovery follows exact release-tag review; generated CLI reference, operational contract, automation examples, security model, constrained rebuild recipe, fail-closed POSIX and PowerShell consumer verification, and release-gate descriptions track current behavior (Unreleased) | Keep the eventual versioned doctor record separate from human diagnostic output |
