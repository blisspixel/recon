...
| Hedged output: sparse evidence stays qualified, dense evidence never absolute | The 80% credible interval (`recon_tool/bayesian.py::_credible_interval`) with the loaded `calibration.min_n_eff` floor from `bayesian_network.yaml`, the `sparse` flag, and the CAL14 declarative-absence model | `test_node_stability_criteria` (per-node stability criteria), `tests/test_interval_coverage.py` (interval width absorbs the elicitation band), `tests/test_bayesian_evidence_groups.py` (CAL7/CAL14 semantics) | Evidence-responsive, not ground-truth calibrated; the wording discipline is CAL13 |
