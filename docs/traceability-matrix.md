...
| Public validation artifacts are reserved synthetic or aggregate-only | Validation hygiene admits reserved examples and rejects non-reserved command examples and corpus lines; public validation fixtures use explicit synthetic sentinels under reserved namespaces | `test_validation_hygiene::test_recon_example_with_real_domain_fails`, `test_validation_hygiene::test_validation_corpus_lines_must_be_reserved_or_synthetic`, `test_validation_hygiene::test_synthetic_and_reserved_validation_artifacts_pass`, `test_validation_fixture_generators::test_synthetic_corpus_source_emits_only_public_safe_identities`, `test_validation_fixture_generators::test_aggregate_source_emits_only_public_safe_identities` | The automated semantic scan is strongest in validation artifacts; older fictional prose examples outside validation remain review-held documentation debt |
| Observations are hedged and provenance-bearing; sparse or degraded evidence lowers confidence or remains unresolved | Collection views mask unavailable channels, built-in insights retain exact generation-time rule and evidence-or-scope associations, confidence is distinct-source and same-claim scoped, explanation records distinguish exact, exact-rule-only, reconstructed, and unsupported lineage, and generated exposure copy uses neutral templates | `tests/test_collection_view.py`, `tests/test_insight_lineage.py`, `tests/test_bayesian_collection_missingness.py`, `tests/test_explanation_coverage.py`, `tests/test_explanation_lineage.py`, `test_exposure::TestProperty8NeutralGeneratedCopy` | Schema-version-1 `provenance_complete` proves graph reachability. `exact_provenance_complete` and `lineage_disconnected_terminals` separately identify terminals lacking an explicit generation-time evidence-to-rule path; reconstructed confidence, legacy insight, and proxy posture associations remain honestly disconnected there. |
...
| Artifact builds select exact uv and a hash-locked build group whose frozen export is committed and shipped in the sdist | `build-constraints.txt`; `test_build_constraints::test_build_root_and_uv_are_exactly_selected`; `test_build_constraints::test_build_constraints_match_frozen_build_group`; `test_package_invariants::test_sdist_retains_public_sources_and_excludes_private_artifacts` |
| Same source and fixed epoch produce matching sdist and reconstructed-wheel artifacts across two constrained builds in one CI job; that wheel feeds blocking cross-platform smoke | `ci.yml` reproducible-build and installed-wheel-smoke jobs; `test_check_script::test_reproducible_build_feeds_blocking_cross_platform_artifact_smoke` |
...
| PyPI must expose exactly the sealed wheel and sdist through bounded, exact metadata with trusted file URLs and matching SHA-256 digests before GitHub assets can be created or replaced, and remote readiness reuses the same metadata validator and repeats the comparison | `scripts/check_release_channel_parity.py`; `test_release_channel_parity::test_public_metadata_validator_returns_only_the_exact_safe_pair`; `test_release_channel_parity::test_exact_pair_returns_shared_digests`; `test_release_workflow_contract::TestPublishedChannelParity`; `test_release_readiness::test_release_channel_parity_reports_exact_digest_evidence` |
