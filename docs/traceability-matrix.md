...
| The sealed wheel executes through both installed entry points in a separate read-only job before PyPI or GitHub publication | `release.yml` package-smoke job; `test_release_workflow_contract::TestPackageSmokeJob`; `test_check_script::test_reproducible_build_smokes_built_wheel_entry_points` |
