...
|---|---|---|---|
| No active scanning, port probes, or zone transfers; default target interaction is limited to the standards-defined MTA-STS policy fetch, and other direct probes are opt-in | Default lookups read DNS, CT logs, provider identity endpoints, and the MTA-STS policy when published; `active_probes` gates Google CSE and BIMI VMC requests | `test_gws_features::TestFetchMtaStsPolicy::test_mta_sts_enforce_mode`, `test_gws_features::TestFetchMtaStsPolicy::test_no_policy_fetch_without_mta_sts_record`, `test_passive_default::test_google_source_passive_by_default_makes_no_cse_probe`, `test_passive_default::test_bimi_vmc_not_fetched_by_default` | MTA-STS is the one default request the target's own servers see; opted-in probes are intentional contact |
| Zero credentials, zero API keys, zero paid APIs | No auth configuration surface exists; every source is an unauthenticated public endpoint; the runtime dependency set in `pyproject.toml` carries no paid-provider SDK | `scripts/check_cost_surface.py` checks runtime code, wheel packages, project dependencies, and workflows; `tests/test_cost_surface.py` pins dependency, runtime-marker, workflow-marker, package-scope, and validation-harness boundaries | The guard is structural and marker-based; a new paid provider marker must be added to the denylist if the ecosystem changes |
| No bundled ML models, embeddings, ASN data, GeoIP data, or local aggregate intelligence database | Packaged data is the fingerprint catalog, schema, signals, profiles, motifs, posture rules, and Bayesian network YAML only; the cohort summary is compute-and-forget (no persistent store) | `tests/test_package_invariants.py` | none material |
| No user-code plugin system: custom fingerprints, signals, profiles, and motifs are data files only | Home-directory overlays are parsed with `yaml.safe_load` and validated structurally; profile and motif overlays are pre-size-capped and bad custom files are skipped without suppressing built-ins | `test_custom_signals::test_custom_signals_are_additive`, `test_motifs::TestUserConfigAdditive`, `test_baseline_anomalies::TestProfileSchemaExtension` | none material |
| Hedged output: sparse evidence stays qualified, dense evidence never absolute | The 80% credible interval (`recon_tool/bayesian_interval.py::credible_interval`) with the loaded `calibration.min_n_eff` floor from `bayesian_network.yaml`, the `sparse` flag, and the CAL14 declarative-absence model | `test_node_stability_criteria` (per-node stability criteria), `tests/test_interval_coverage.py` (interval width absorbs the elicitation band), `tests/test_bayesian_evidence_groups.py` (CAL7/CAL14 semantics) | Evidence-responsive, not ground-truth calibrated; the wording discipline is CAL13 |
