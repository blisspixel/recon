...
| Observations are hedged and provenance-bearing; sparse or degraded evidence lowers confidence or remains unresolved | Collection views mask unavailable channels, built-in insights retain exact generation-time rule and evidence-or-scope associations, confidence is distinct-source and same-claim scoped, explanation output reports disconnected terminals, and generated exposure copy uses neutral templates | `tests/test_collection_view.py`, `tests/test_insight_lineage.py`, `tests/test_bayesian_collection_missingness.py`, `tests/test_explanation_coverage.py`, `test_exposure::TestProperty8NeutralGeneratedCopy` | `provenance_complete` proves reachability in the reconstructed graph, not exact generation-time lineage for signal-adjacent, conflict, lexical, or posture associations that remain reconstructed |
