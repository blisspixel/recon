...
recon **v2.13.0** is the current production baseline: CLI, versioned JSON,
...
| **v2.14.0** | Catalog quality loop - **active build priority** | Independent of fusion promotion; rank and regional are closed, and the vendor-seed label, denominator, exclusion, disclosure, and fail-closed tooling contract is implemented | Freeze the private provider source dossier and exact disjoint frame, publish commitments before collection, complete provider-level corroboration evidence, then re-observe the frozen prior sample for drift; broad catalog growth remains gated by those rounds |
...
vendor-seed protocol and fail-closed preparer and evaluator are implemented.
Next, freeze the private provider-controlled source dossier and exact disjoint
frame, publish their aggregate commitments before target contact, then collect
and report provider-level relationship corroboration. This measure is not
recall because a provider relationship does not label publication of a
particular DNS record. Drift follows.
