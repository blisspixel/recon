...
class TestBackwardCompatDetectProvider:
    ...
    Account slugs stay account indicators. An unmatched MX record is reported
    separately as custom or unclassified, without inferring who operates it.
    """

    def test_microsoft365_slug_only_no_mx(self) -> None:
        """No MX records keep the account indicator separate from routing."""
        result = detect_provider(services=(), slugs=("microsoft365",), has_mx_records=False)
        assert result == "Microsoft 365 (account indicator; no MX observed)"
    ...
    def test_google_workspace_slug_only(self) -> None:
        ...
        assert result == "Google Workspace (account indicator; no MX observed)"
